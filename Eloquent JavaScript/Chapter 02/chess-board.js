...
// The underscore character was used instead of space, so that the "space"
// would be visually defined.
var space = "_";
...
for(var row = 0; row < size; row++) {
    for(var column = 0; column < size; column++) {
        if(((column + row) % 2) === 0) {
            grid = grid.concat(character);
        } else {
            grid = grid.concat(space);
        }
    }
    
    grid = grid.concat(newline);
}
